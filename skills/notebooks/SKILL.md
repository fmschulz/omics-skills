...
4. **Load data with project-relative paths.** Prefer DuckDB for TSV/Parquet (`duckdb.read_csv`, `duckdb.read_parquet`). Avoid absolute paths and `~`. Avoid hidden state from the runtime working directory.

5. **Run all cells to generate plots.** Execute the notebook headlessly on a fresh kernel before delivery:
   - Marimo: `uv run marimo export ipynb <notebook.py> -o <notebook.executed.ipynb>` or `uv run marimo run <notebook.py>` for a non-interactive smoke run; for a deterministic HTML artifact, `uv run marimo export html <notebook.py> -o <notebook.html>`.
...
6. **Evaluate the plots, then refine.** This step is required, not optional. After the run-all execution:
...
7. **Deliver pre-executed notebooks.** The artifact handed back to the user must:
...
8. **Convert between marimo and Jupyter** when the user asks for it:
   ...
   - After conversion, re-run step 5 (execute), step 6 (inspect plots), and step 7 (deliver pre-executed).
...
- [ ] For marimo: `uvx marimo check <notebook.py>` reports no errors.
