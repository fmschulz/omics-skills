...
for skill in bio-logic bio-foundation-housekeeping scientific-writing crossref-lookup scientific-impact-assessment beautiful-data-viz; do
...
if [ -d "$HOME/.codex" ]; then
    ...
    if [ -d "$HOME/.codex/agents" ]; then
        count=$(find "$HOME/.codex/agents" \( -name "omics-scientist.md" -o -name "literature-expert.md" -o -name "science-writer.md" -o -name "dataviz-artist.md" \) 2>/dev/null | wc -l)
        if [ "$count" -eq 4 ]; then
